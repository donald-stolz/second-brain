...
4. **Plan tomorrow**: Create `Todo-{tomorrow's date}.md` in `00 Inbox/Daily Plan/` from `Templates/Todo Note.md`:
   - Fill `created`/`modified` with tomorrow's date, `status: active`, leave `summary`/`tags` for the user.
...
5. **Close out the reviewed note**: Once tomorrow's note is written, update the reviewed note's `status` to `done` and bump its `modified` field to today's date. This is the only write to the reviewed note in the whole flow.
